...
export default function PageHeader(props: any) {
      ...
      <header className="root-page-header page-section-big" style={props.style}>
        <div className="root-page-header-container">
          <div className={`align ${align}`}>
            <h1 className="root-page-title font-article">{title}</h1>

            {quote != null ? (
              <>
                <div className="brief font-article">
                  <p className="caption brief-paragraph">「{quote}」</p>
                  {quoteFrom != null ? (
                    <>
                      <p className="caption brief-paragraph">——{quoteFrom}</p>
                    </>
                  ) : (
                    ""
                  )}
            ...
            {brief != null ? (
              <>
                <div className="brief secondary font-article">
                  <p className="caption brief-paragraph">{brief}</p>
                </div>
              </>
            ) : (
              ""
  ...
  return <>{showHeader != false ? element : ""}</>;
}
